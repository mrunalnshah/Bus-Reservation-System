/*
This module cancel as many seat as per user inputs and fetch all data into array and null the seat number if its registered else its EMPTY.
...
void admin_buscancellation();

/*
it fetch the data of passengers and seat from the filename based on the respective busnumber and stores them in array. ask the user to cancel number of seats and
puts in loop to cancel each seat if occupied and store them into array if not occupied then prints EMPTY Already and no changes to the array. at last it runs a loop to
overwrite and print all the data from array to filename based on busnumber.
*/
void admin_buscancellation(){
    struct date{
        int day;
            ...
            if(strcmp(choice_busnumber,busnumber_read) == 0){
                ...
                if(date_verify == 1){
                    ...
                    if(time_verify == 1){
                        ...
                        bus_layout(choice_busnumber);
                        printf("\t\t***************************************************\n");
                        printf("\t\t            REFUND OF AMOUNT %0.5d IS INITIATED      \n ",refund_amount);
                        printf("\t\t***************************************************\n");
